...
export function getGraphStyles(): Stylesheet[] { return [
  ...
  { selector:'node[type = "publisher"]', style:{'background-color':NC.publisher,shape:'square'} }, { selector:'node[type = "publication"]', style:{'background-color':NC.publication,shape:'square'} },
  ...
  { selector:'edge[type = "has_genre"]', style:{'line-color':NC.genre,opacity:0.4} }, { selector:'edge[type = "has_category"]', style:{'line-color':NC.category,opacity:0.4} },
  { selector:'edge[type = "written_by"]', style:{'line-color':NC.author,'line-style':'dashed'} }, { selector:'edge[type = "illustrated_by"]', style:{'line-color':NC.artist,'line-style':'dashed'} },
  { selector:'edge[type = "published_by"]', style:{'line-color':NC.publisher} }, { selector:'edge[type = "serialized_in"]', style:{'line-color':NC.publication} },
  { selector:'edge[type = "related_to"]', style:{'line-color':NC.series,width:2} }, { selector:'edge[type = "recommended_with"]', style:{'line-color':'#FFD700',width:'mapData(weight,0,10,1,4)',opacity:0.5} },
  { selector:'edge.highlighted', style:{opacity:0.8,width:2} }, { selector:'edge.faded', style:{opacity:0.05} },
]; }
