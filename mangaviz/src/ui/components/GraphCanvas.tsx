...
const GC:React.FC<Props>=({graphData,layoutData,nodeTypeFilters,edgeTypeFilters,clusterFilter,selectedNodeId,focusToken=0,onNodeClick,isLoading,isAtlasMode})=>{
  ...
  const eles=useMemo(()=>{const e:cytoscape.ElementDefinition[]=[];for(const n of fltrd.nodes){const p:{x?:number;y?:number}={};const lp=layoutData?.positions[n.id];if(n.x!==undefined){p.x=n.x;p.y=n.y;}else if(lp){p.x=lp.x;p.y=lp.y;}e.push({group:'nodes',data:{id:n.id,label:n.label,type:n.type,weight:n.weight,cluster:n.cluster},position:p.x!==undefined?{x:p.x,y:p.y}:undefined});}for(const ed of fltrd.edges)e.push({group:'edges',data:{id:ed.id,source:ed.source,target:ed.target,type:ed.type,weight:ed.weight}});return e;},[fltrd,layoutData]);
  const layout = useMemo(() => layoutData ? { name:'preset' } : LIVE_LAYOUT, [layoutData]);
  const graphKey = useMemo(() => layoutData ? 'atlas-graph' : `live-${fltrd.nodes.length}-${fltrd.edges.length}-${fltrd.nodes[0]?.id || 'empty'}-${fltrd.edges[0]?.id || 'empty'}`, [layoutData, fltrd.nodes, fltrd.edges]);
