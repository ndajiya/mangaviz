...
const StatsPanel: React.FC<Props> = ({ stats, visible, onToggle }) => {
  if (!stats) return <div className="stats-toggle" onClick={onToggle}><span>Stats</span></div>;
  return (
    <div className={`stats-panel ${visible?'visible':''}`}>
      <div className="stats-header"><h3>Statistics</h3><button className="close-btn" onClick={onToggle}>&times;</button></div>
      <div className="stats-content">
        <div className="stat-section"><h4>Overview</h4><div className="stat-grid">
          {[{l:'Nodes',v:stats.totalNodes},{l:'Edges',v:stats.totalEdges},{l:'Series',v:stats.seriesCount},{l:'Genres',v:stats.genreCount},{l:'Categories',v:stats.categoryCount},{l:'Authors/Artists',v:stats.authorArtistCount},{l:'Publishers',v:stats.publisherCount}].map(s=><div key={s.l} className="stat-item"><span className="stat-value">{s.v.toLocaleString()}</span><span className="stat-label">{s.l}</span></div>)}
        </div></div>
