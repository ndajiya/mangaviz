...
- After replacing the catch-all route with a concrete function, production remains reproducible but now fails with `net::ERR_CONTENT_DECODING_FAILED`.
- That symptom strongly indicates a proxy encoding mismatch: the function is likely returning a decoded response body while still forwarding upstream compression headers such as `content-encoding` and/or `content-length`.
...
- Updated the concrete proxy to request `identity` encoding upstream, buffer the upstream body explicitly, and strip `content-encoding` and `content-length` from the downstream response before returning it to the browser.
